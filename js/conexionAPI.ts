...
async function recibirDatosAPI() {
    try {
      ...
      const url: string = "https://alurageek-api-flax.vercel.app/Items";
...
async function crearNuevoItem(name: string, price: string, image: string) {
    ...
    const url2: string = "https://alurageek-api-flax.vercel.app/Items";
    try {
...
export async function eliminarItem(id:string|number) {
    ...
    const url: string = `https://alurageek-api-flax.vercel.app/Items/${id}`;
    try {
